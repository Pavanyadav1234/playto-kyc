...
If someone sends a 50MB file, the size check catches it and returns a 400 error with the exact file size. The check happens server-side so the client cannot bypass it.
